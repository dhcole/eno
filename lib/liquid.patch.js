// Via https://github.com/prose/prose/blob/master/vendor/liquid.patch.js
// Updated to not require Backbone collections
...
module.exports = function() {
  Liquid.readTemplateFile = (function(path) {
    var url = function(path) {
      return config.api_host + '_includes/' + path + config.access_token;
    }
    var content = $.ajax({
      type: 'GET',
      async: false,
    ...
    return (content) ? content : console.warn("File Not Found:" + path);
  }).bind(this);
  ...
  Liquid.Template.registerTag( 'highlight', Liquid.Block.extend({
    ...
    render: function(context) {
      ...
      return '<pre title="Highlight formatting is disabled in preview.">' + output[0] + '</pre>';
    }
  }));
